...
async function createSubmission({ submission, analysis }) {
  const store = await readStore();
...
async function updateSubmission(id, { submission, analysis }) {
  const store = await readStore();
...
async function deleteSubmission(id) {
  ...
  store.submissions = store.submissions.filter((record) => record.id !== id);
...
async function listProfiles() {
  ...
  store.submissions.forEach((record) => {
