...
async function fetchHistory() {
  const [historyResponse, analyticsResponse] = await Promise.all([fetch("/api/feedbacks"), fetch("/api/analytics")]);
...
async function updateSelectedFeedback() {
  ...
  try {
    elements.feedbackForm.elements.namedItem("reviewerName").value = state.reviewerName;
    const payload = formToPayload(elements.feedbackForm);
    const response = await fetch(`/api/feedbacks/${state.selectedFeedback.id}`, {
      method: "PATCH",
      headers: {
        "Content-Type": "application/json"
      },
...
function openSelectedReport() {
  ...
  window.open(`/api/feedbacks/${state.selectedFeedback.id}/report`, "_blank", "noopener");
}

function exportVaultCsv() {
  window.location.href = "/api/feedbacks/export.csv";
}
...
async function initialize() {
  elements.reviewerNameInput.value = state.reviewerName;
  elements.feedbackForm.elements.namedItem("reviewerName").value = state.reviewerName;
  ...
  document.addEventListener("click", async (event) => {
    ...
    if (reportButton) {
      window.open(`/api/feedbacks/${reportButton.dataset.openReport}/report`, "_blank", "noopener");
    }
  });
